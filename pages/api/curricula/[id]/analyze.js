...
const REGIONAL_HIGH_GROWTH_INDUSTRIES_OK = [
  { 
    ...
    keywords: ["health", "biology", "chemistry", "anatomy", "psychology", "cna", "medical"],
    skills: ["Patient Care Fundamentals", "Basic Medical Terminology", "Empathy & Communication", "Scientific Literacy (Biology/Chemistry)"] 
  },
  { 
    id: "manufacturing", 
    name: "Manufacturing (including Advanced)", 
    keywords: ["manufacturing", "engineering", "tech", "robotics", "cad", "shop", "industrial"],
    skills: ["Technical Aptitude", "Problem-Solving", "Basic Math/Physics Application", "Intro to Design/CAD (awareness)", "Safety Protocols"] 
  },
  { 
    id: "retail", 
    name: "Retail Trade", 
    keywords: ["business", "marketing", "sales", "economics"],
    skills: ["Customer Service Principles", "Basic Sales Techniques", "Communication", "Inventory Awareness (basic math)"] 
  },
  { 
    id: "professional", 
    name: "Professional, Scientific, & Technical Services", 
    keywords: ["business", "accounting", "it", "computer science", "research", "analysis"],
    skills: ["Analytical Thinking", "Basic IT Literacy/Computer Science", "Research Skills", "Professional Communication"] 
  },
];
...
const generateEnhancedAnalysisResults = (curriculum) => {
  ...
  if (!curriculum) {
    ...
    return { error: "Curriculum data not provided for analysis." };
  }
  ...
  let analysis = {
    ...
    analyzedBy: "EnhancedMockEngine V2.1 (USAO Focus Debug)",
    overallAlignmentScore: 0,
    overallStatusText: "Analysis Pending",
    standardAlignmentDetails: {
      summary: "",
      findings: [],
      overallScore: 0, // Ensure this is initialized
      overallStatusText: "Analysis Pending", // Ensure this is initialized
    },
    gapAnalysis: {
      summary: "",
      identifiedGaps: [],
  ...
  try {
    ...
    console.log("[generateEnhancedAnalysisResults] Analyzing USAO Admissions Requirements...");
    ...
    Object.keys(USAO_ADMISSIONS_REQUIREMENTS).forEach(key => {
      ...
      if (met === "Gap") {
        admissionsGaps.push({ id: `gap-adm-${key}`, area: `HS Req: ${req.label}`, description: `Curriculum does not clearly indicate coverage of the required ${req.required} units.`, severity: "High" });
      }
      totalScore += points;
    });
    ...
    console.log("[generateEnhancedAnalysisResults] Analyzing USAO Intro Course Themes...");
    ...
    Object.keys(USAO_INTRO_COURSE_THEMES).forEach(subjectArea => {
        USAO_INTRO_COURSE_THEMES[subjectArea].forEach((theme, index) => {
            maxPossibleScore += 1;
            // Ensure theme is a string before calling split
    ...
    if (introCourseGaps.length > 0) {
      analysis.gapAnalysis.summary = (analysis.gapAnalysis.summary ? analysis.gapAnalysis.summary + " " : "") + `Gaps also identified in preparedness for some USAO introductory course themes.`;
    } else {
      analysis.gapAnalysis.summary = (analysis.gapAnalysis.summary ? analysis.gapAnalysis.summary + " " : "") + `Appears to provide a good foundation for USAO introductory courses.`;
    }

    // 3. Regional Industry Alignment
    console.log("[generateEnhancedAnalysisResults] Analyzing Regional Industry Alignment...");
    REGIONAL_HIGH_GROWTH_INDUSTRIES_OK.forEach(industry => {
      maxPossibleScore += 3;
      let alignmentScorePercent = 20;
      let alignmentStatusText = "Limited Alignment";
      let keySkillsCovered = [];
      let identifiedGaps = [`Further development needed for specific ${industry.name} pathways.`];
      let opportunities = [`Explore partnerships with local ${industry.name} employers.`, `Integrate industry-specific project options.`];
      let matchCount = 0;

      (industry.keywords || []).forEach(keyword => { // Ensure keywords array exists
      ...
      if (alignmentScorePercent >= 75) alignmentStatusText = "Strong Alignment";
      else if (alignmentScorePercent >= 50) alignmentStatusText = "Moderate Alignment";
      else if (alignmentScorePercent >= 30) alignmentStatusText = "Some Alignment";
      
      analysis.regionalIndustryAlignment.curriculumAlignmentWithKeyIndustries.push({
        id: `ind-align-${industry.id}`, // Added id for key prop
        industrySector: industry.name,
        relevantPathways: `Based on keywords: ${(industry.keywords || []).join(', ')}`,
        alignmentScorePercent: alignmentScorePercent,
        alignmentStatusText: alignmentStatusText,
        keySkillsCovered: keySkillsCovered.length > 0 ? keySkillsCovered : ["General Foundational Skills"],
    ...
    console.log("[generateEnhancedAnalysisResults] Calculating Overall Score...");
    ...
    console.log("[generateEnhancedAnalysisResults] Analysis object generated:", JSON.stringify(analysis, null, 2).substring(0, 500) + "..."); // Log snippet of result

  } catch (e) {
    console.error("[generateEnhancedAnalysisResults] Error during generation:", e);
    analysis.error = "Error occurred during analysis generation: " + e.message;
    analysis.overallStatusText = "Analysis Failed";
    // Ensure all parts of analysis object are still valid even if partially filled
    analysis.standardAlignmentDetails = analysis.standardAlignmentDetails || { summary: "", findings: [] };
    analysis.gapAnalysis = analysis.gapAnalysis || { summary: "", identifiedGaps: [] };
    analysis.regionalIndustryAlignment = analysis.regionalIndustryAlignment || { region: "", summary: "", topHighGrowthIndustries: [], curriculumAlignmentWithKeyIndustries: [] };
...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    try {
      ...
      const analysisDataObject = generateEnhancedAnalysisResults(curriculum);
      
      if (analysisDataObject.error) {
        console.error(`[API /analyze] Error from generateEnhancedAnalysisResults: ${analysisDataObject.error}`);
        // Still save what we have, but indicate partial failure if desired, or return error
        // For now, we'll save it as is, the error field will be in analysisResults
      }

      console.log(`[API /analyze] Stringifying analysis data for ${id}...`);
      const analysisDataString = JSON.stringify(analysisDataObject);

      console.log(`[API /analyze] Updating curriculum ${id} in database...`);
      const updatedCurriculum = await prisma.curriculum.update({
        where: { id: String(id) },
        data: {
          analysisResults: analysisDataString,
        },
      });
      console.log(`[API /analyze] Curriculum ${id} updated successfully.`);

      // Parse back for the response to ensure client gets an object
      let parsedAnalysisResults = {};
      try {
        if (updatedCurriculum.analysisResults) {
          parsedAnalysisResults = JSON.parse(updatedCurriculum.analysisResults);
        }
      } catch (parseError) {
        console.error(`[API /analyze] Failed to parse analysisResults from DB for response: ${parseError}`);
        // Send the string if parsing fails, or handle error
        parsedAnalysisResults = { error: "Failed to parse analysis results for response." };
      }
      ...
      res.status(200).json({ message: "Enhanced analysis complete and results saved.", curriculum: serializedUpdatedCurriculum });
      ...
      // Send a more generic error to the client but log specifics on the server
      res.status(500).json({ error: "Unable to analyze curriculum due to a server error. Check server logs." });
    }
  } else {
